...
5. A small web UI on the router lets you switch modes and pick the time, and shows how much of the current window is left. It also shows your **5h and 7-day usage** (percent utilized + reset time), since Pro/Max subscriptions are capped by both. It's linked from LuCI under **Services → Claude Warmup**.
   ...
   These usage numbers are only as fresh as the *last ping* — they don't update in real time. The cron check (every 5 min) and the web UI's own polling (every 15s) never call Anthropic themselves; only an actual warm-up ping does, which happens roughly once per 5h window (or once at the fixed-mode start time, then every ~5h after). If you use Claude actively between pings, that usage won't show up here until the next one fires. **"Ping now"** fetches current numbers on demand, but it's a real API request — Haiku, capped at 16 output tokens, so tiny — and it does consume a small amount of your actual quota. It's not a free refresh; use it when you actually want to check or force a warm-up, not as a casual "just curious" button.
